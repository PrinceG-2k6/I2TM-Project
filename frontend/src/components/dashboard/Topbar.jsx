      ...
      <div className="flex items-center gap-4">
        ...
        <div ref={dropdownRef} className="relative w-70">
          ...
          >
            ...
            <input
              ...
              onChange={(e) => { setCitySearchInput(e.target.value); if (!isCitySearchOpen) setIsCitySearchOpen(true); }}
          ...
          {isCitySearchOpen && (
            <div className="absolute top-[calc(100%+8px)] left-0 w-90 bg-(--color-5) rounded-md border border-(--color-3) z-100 p-3 animate-[fadeIn_0.18s_ease_forwards]">
              <button
                type="button"
                onClick={() => { detectUserLocation(); setIsCitySearchOpen(false); }}
              ...
              <div className="text-xs mb-2">Cities ({filteredCities.length})</div>
                  ...
                  filteredCities.map((city) => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between px-3 py-2 rounded-md cursor-pointer transition-all duration-120 ease-in-out bg-(--color-4) ${isSelected ? 'border border-(--color-3)' : 'border border-transparent hover:opacity-75'}`}
                      >
                        <div>
                          <div className={`text-sm ${isSelected && 'text-(--color-6)'}`}>{city.name}</div>
                          <div className="text-[11px] text-slate-500">{city.state} • {city.junctions.length} Active Nodes</div>
                        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2.5">
            <span className="text-xs">{activeCity?.name || 'No location'}</span>
            {userLocationDetected && (
              <span className="text-[10px] bg-(--color-3) text-(--color-2) px-2 pt-0.5 pb-px rounded-full">GPS</span>
            )}
          </div>
        </div>
      </div>
